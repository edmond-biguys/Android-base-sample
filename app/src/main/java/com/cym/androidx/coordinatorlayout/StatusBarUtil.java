...
public class StatusBarUtil {
    ...
    public static void setColor(Activity activity, @ColorInt int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (fakeStatusBarView != null) {
                ...
                decorView.addView(createStatusBarView(activity, color, 255));
            }
    ...
    public static void setDrawable(Activity activity, @DrawableRes int drawableRes) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            activity.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            activity.getWindow().setBackgroundDrawableResource(drawableRes);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            ViewGroup decorView = (ViewGroup) activity.getWindow().getDecorView();
            View fakeStatusBarView = decorView.findViewById(FAKE_STATUS_BAR_VIEW_ID);
            if (fakeStatusBarView != null) {
                ...
                fakeStatusBarView.setBackgroundResource(drawableRes);
            } else {
                decorView.addView(setStatusBarView(activity, drawableRes));
            }
            setRootView(activity);
        }
    }
     ...
     *
     * @param activity    需要设置的activity
     * @param drawableRes 状态栏颜色值
     * @return 状态栏矩形条
     */
    private static View setStatusBarView(Activity activity, @DrawableRes int drawableRes) {
        // 绘制一个和状态栏一样高的矩形
        View statusBarView = new View(activity);
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity));
